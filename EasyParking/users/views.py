...
def new_user(request : HttpRequest):
    if request.method == "POST":
     ...
     if profile.is_owner == True:
         return redirect("users:add_place")

     elif profile.is_owner == False:
        return redirect("users:all_places")
...
def information_and_reserve(request : HttpRequest):
    if request.method == "POST":
      ...
      user_reserve.save()
      return redirect ("users:success_resservation", user_reserve.id)
...
def owner_manage(request : HttpRequest):
    ...
    return render(request , "user/owner_manage.html" , {"users_reservation": all_reservation})



def success_resservation(request : HttpRequest, reserve_id):

    return render(request , "user/success_reservation.html")
